const getTeamById = () => {
  ...
  return new Promise(function(resolve, reject) {
      ...
      caches.match(base_uri + "/teams/" + idParam).then(function(response) {
        if (response) {
          response.json().then(function(data) {
            ...
            var articleHTML = getTeamArticle(data);
    ...
    fetchAPI(base_uri + "/teams/" + idParam)
      .then(function(data) { // Menyusun komponen card artikel secara dinamis
        var articleHTML = getTeamArticle(data);
        // Sisipkan komponen card ke dalam elemen dengan id #content
        document.getElementById("body-content").innerHTML = articleHTML;
        loader.style.display = 'none'
...
const getSavedTeams = () => {
  getAll().then(function(teams) {
    ...
    if (isObjectEmpty(teams)) {
      articlesHTML = getTeamSavedEmpty();
    } else {
      articlesHTML = getTeamSavedArticle(teams);
    }
...
const getTeamSavedArticle = (teams) => {
  var articlesHTML = "";
